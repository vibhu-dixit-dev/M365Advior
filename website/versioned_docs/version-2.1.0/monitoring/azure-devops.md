...
# <IIcon icon="vscode-icons:file-type-azurepipelines" height="48" /> Set up M365Advisor in Azure DevOps

This guide will walk you through setting up M365Advisor in Azure DevOps and automate the running of tests using Azure DevOps Pipelines.

## Why Azure DevOps?

Azure DevOps is a great way to automate the daily running of M365Advisor tests to monitor your tenant. You can use Azure DevOps to run M365Advisor tests on a schedule, such as daily, and view the results in the Azure DevOps interface.

Azure DevOps comes with a [free tier](https://azure.microsoft.com/pricing/details/devops/azure-devops-services/) that includes 1,800 minutes of M365Advisor test runs per month (unlimited hours if you use a self-hosted agent).
...
## Set up the M365Advisor repository in Azure DevOps
...
- If this is your first time using Azure DevOps, you will first need to create an organization.
  - [Azure DevOps - Create an organization](https://learn.microsoft.com/azure/devops/organizations/accounts/create-organization)
    ...
    :::
- Create a new project to host your M365Advisor tests and Azure Pipeline.
  - [Azure DevOps - Create a project](https://learn.microsoft.com/azure/devops/organizations/projects/create-project)

### Import the M365Advisor Tests repository

- Select **Repos** from the left-hand menu
- Select the **Import** button in the **Import a repository** section
- Enter the URL of the M365Advisor repository `https://github.com/m365advisor365/m365advisor-tests`
...
- Select **+ Create**.
- Enter a name for the resource group (e.g. `M365Advisor-Resource-Group`)
- Select any region
...
- Specify the following parameters:
  ...
  - **Resource Group**: Select the resource group created in the previous step. (e.g. `M365Advisor Resource Group`) Leaving this field empty will grant Contribute access to all resources in the subscription.
  - **Service connection name**: A name for this connection (e.g. `M365Advisor Service Connection`)
- Select **Save** to create the connection.

### Grant permissions to Microsoft Graph

- Select the service connection you created in the previous step (e.g. `M365Advisor Service Connection`)
...
- Select the repository where you imported the M365Advisor tests
- Select **Starter pipeline**
- Replace the content of the `azure-pipelines.yml` file with the code below
- Verify the `azureSubscription` value is set to the service connection you created in the previous step (e.g. `M365Advisor Service Connection`)
...
steps:
  - task: AzurePowerShell@5
    displayName: "Run M365Advisor"
    inputs:
      azureSubscription: "M365Advisor Service Connection"
      pwsh: true
      ...
      Inline: |
        ...
        Invoke-M365Advisor -Path $(System.DefaultWorkingDirectory)/tests/M365Advisor/ -PesterConfiguration $PesterConfiguration -OutputFolder '$(System.DefaultWorkingDirectory)/test-results'
  - publish: $(System.DefaultWorkingDirectory)/test-results
    displayName: Publish M365Advisor Html Report
...
pool:
  name: M365Advisor-DevOps

jobs:
  - job: InstallPowerShell
    displayName: "Running M365Advisor on VMSS"
    steps:
      - script: |
...
steps:
  - pwsh: |
    ...
    displayName: Run M365Advisor Tests
  - publish: $(System.DefaultWorkingDirectory)/test-results
    artifact: TestResults
    displayName: Publish M365Advisor Html Report
  - task: PublishTestResults@2
...
<iframe width="686" height="386" src="https://www.youtube.com/embed/42FADXWrDBo" title="M365Advisor Azure DevOps integration" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
The M365Advisor report can be downloaded and viewed by selecting the **Published** artifact.

![Screenshot of the downloaded M365Advisor report](assets/azure-devops-m365advisor-report.png)
...
In the **Summary** tab select on any of the errors to view the raw logs from M365Advisor.

![Screenshot of Azure DevOps Pipeline Logs Page](assets/azure-devops-logs-page.png)

## Keeping your M365Advisor tests up to date

The M365Advisor team will add new tests over time. To get the latest updates, use the commands below to update your Azure repository with the latest tests.

- Clone your fork of the **m365advisor-tests** from Azure DevOps to your local machine. See [Clone an existing Git repo](https://learn.microsoft.com/azure/devops/repos/git/clone).
