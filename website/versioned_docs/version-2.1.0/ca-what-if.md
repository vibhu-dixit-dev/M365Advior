...
## Conditional access regression testing with M365Advisor

The M365Advisor framework allows you to define tests that can be run against your Conditional Access policies using the What If API. The tests can be run as part of your daily automation tests and when you make changes to your policies.
...
M365Advisor tests written using this API may need to be updated as the API moves towards v1.0.

Please make sure you have the latest version of M365Advisor installed.
:::

## Writing Conditional Access What-If tests

The M365Advisor PowerShell module includes the **Test-MtConditionalAccessWhatIf** cmdlet that allows you to run What-If tests against your Conditional Access policies.
...
A M365Advisor test can be written to simulate a user sign in and check if a specific Conditional Access policy is enforced.
...
- To learn more about the **Test-MtConditionalAccessWhatIf** cmdlet, including the supported parameters and examples see [Test-MtConditionalAccessWhatIf | M365Advisor Reference](https://m365advisor.dev/docs/commands/Test-MtConditionalAccessWhatIf).
- For a step by step guide on writing custom M365Advisor tests and running them see [Writing M365Advisor tests](/docs/writing-tests).
