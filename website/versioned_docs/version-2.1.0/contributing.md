...
This guide is for anyone who wants to contribute to the M365Advisor project. Whether you want to contribute to the code, documentation, or just have an idea for a new feature, we welcome your input.

Follow the guide below to set up M365Advisor for development on your local machine, make changes, and submit a pull request.

## M365Advisor PowerShell module dev guide
...
- Load the PowerShell module. This needs to be done anytime you make changes to the code in `./powershell`.
  - `Import-Module ./powershell/M365Advisor.psd1 -Force`
- Run M365Advisor
  - `Invoke-M365Advisor`

### Pester Tests

- Tests for the M365Advisor module are at /powershell/tests
...
We have the following [test](https://github.com/m365advisor365/m365advisor/tree/main/tests) folders:

- `/CISA` - CISA tests
- `/Custom` - Folder for user's to add tests (do not add any tests to this folder).
- `/EIDSCA` - EIDSCA tests
- `/M365Advisor` - M365Advisor tests
  - `/Entra` - M365Advisor's Entra tests
...
- [x] Check if your new idea is not already covered by an existing test (you can also jump on the [M365Advisor Discord](https://discord.entra.news) to discuss)
- [x] Reserve a unique id for your test (e.g. `Invoke-M365Advisor -Tag MS.AAD.5.1`). See [#️⃣ Pick next M365Advisor test sequence number](https://github.com/m365advisor365/m365advisor/issues/697) on how to reserve a unique id for your new test.
...
- [x] The related cmdlet for the test has a .md file to explain the test in detail and provides all the context required for the user to resolve the issue, including deep links to the admin portal page to resolve the issue. This will be shown to the user when they view the test report. The file should include:
  ...
  - [x] If there are multiple objects (e.g. list of CA policies, Users, etc) then use the `GraphObjects` and `GraphObjectType` parameters in [Add-MtTestResultDetail](https://github.com/m365advisor365/m365advisor/blob/main/powershell/public/Add-MtTestResultDetail.ps1). These include deep links to the admin portal. If the object type you wish is not available you can add it to [Get-GraphObjectMarkdown.ps1](https://github.com/m365advisor365/m365advisor/blob/main/powershell/internal/Get-GraphObjectMarkdown.ps1). Feel free to ask on Discord if you need help with this.
  - [x] If the test is about a specific setting the message should link to the page where the setting can be configured. The .md file should also include steps to configure the setting as well as a link to the admin portal. For a good example of a well written error page see [Test-MtCisaWeakFactor.ps1](https://github.com/m365advisor365/m365advisor/blob/main/powershell/public/cisa/entra/Test-MtCisaWeakFactor.ps1). Another good example is [Test-MtCisaAppUserConsent.ps1](https://github.com/m365advisor365/m365advisor/blob/main/powershell/public/cisa/entra/Test-MtCisaAppUserConsent.ps1) and the related [Test-MtCisaAppAdminConsent.md](https://github.com/m365advisor365/m365advisor/blob/main/powershell/public/cisa/entra/Test-MtCisaAppAdminConsent.md).

When in doubt always check the existing tests for the conventions used, feel free to discuss on [Discord](https://discord.entra.news) or [GitHub Issues](https://github.com/m365advisor365/m365advisor/issues).
...
- The **PSScriptAnalyzer**, **PSFramework** and **PSModuleDevelopment** modules are required to run the tests, install them with `Install-Module PSFramework, PSModuleDevelopment, PSScriptAnalyzer`
- If a test is not applicable (e.g. it says not to use plural but the product name is AzureDevOps then you can add a `SuppressMessageAttribute` tag (see Invoke-M365Advisor.ps1 which has supression tags at the beginning of the function).
...
The [/build/eidsca/Update-EidscaTests.ps1](https://github.com/m365advisor365/m365advisor/blob/main/build/eidsca/Update-EidscaTests.ps1) script is used to generate the EIDSCA commands in the M365Advisor module along with the EIDSCA tests and documentation.
...
The illustration below shows the workflow for integrating EIDSCA tests and documentation into M365Advisor.

![EIDSCA and M365Advisor integration workflow](assets/eidcsa-m365advisor-workflow.png)
...
```powershell
$affectedFilePath = '/Users/merill/GitHub/m365advisor/powershell/public/m365advisor/entra/Test-MtTenantCreationRestricted.ps1'
$content = Get-Content $affectedFilePath -Raw; $content | Out-File $affectedFilePath -Encoding UTF8BOM
```

## Contributing to M365Advisor docs and blog posts

Simple edits can be made in the GitHub UI by selecting the `Edit this page` link at the bottom of each page or you can browse to the [docs](https://github.com/m365advisor365/m365advisor/tree/main/website/docs) folder on GitHub.
...
The [M365Advisor.dev](https://m365advisor.dev) website is built using [Docusaurus](https://docusaurus.io/).
...
- You can search for icons at [Iconify](https://icon-sets.iconify.design/) and include them in the markdown. See the [Daily Automation](https://m365advisor.dev/docs/automation/) page for examples.
...
There are two versions of the M365Advisor website:

- [Production](https://m365advisor.dev) - This is the live version of the site that is updated whenever a new version of the M365Advisor module is released.
- [Preview](https://preview.m365advisor.dev) - This is the version of the site that is updated with every commit to the main branch. This allows you to see changes before they are published to production.

| Environment | URL                          | Branch       |  Update Trigger                    |
|-------------|------------------------------|--------------| -----------------------------------|
| Production  | https://m365advisor.dev          | website-prod | New M365Advisor module release         |
| Preview     | https://preview.m365advisor.dev  | main         | Every commit to the main branch    |

When a new version of the M365Advisor module is released, the documentation will be updated to reflect the changes in that version.
...
This is because the `website-prod` branch is deleted and recreated with every M365Advisor module release to ensure it is always in sync with the released version of the module.
