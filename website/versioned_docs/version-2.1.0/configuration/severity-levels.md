...
# Severity Levels for M365Advisor Tests

## Severity Framework and Rating

M365Advisor uses Common Vulnerability Scoring System (CVSS) as a method of assessing security risk and prioritization for M365Advisor tests. CVSS is an industry standard vulnerability metric. You can learn more about CVSS at [FIRST.org](https://www.first.org/cvss/user-guide).

In addition to the Critical, High, Medium and Low severity levels, M365Advisor also includes an **Info** severity level. This is used for tests that are not vulnerabilities, but rather provide information about the configuration of a system. No remediation is required for these tests, but they may be useful for understanding the configuration of a system.

### Severity Levels

Every M365Advisor test includes a severity level. This severity level of M365Advisor configuration is based on our self-calculated CVSS score for each specific vulnerability.
...
For CVSS, M365Advisor uses the following severity rating system:
...
M365Advisor allows you to customize the severity levels of the out-of-the-box M365Advisor tests. You can do this by creating a custom configuration file and specifying the severity levels for each test. This allows you to tailor the severity levels to your specific needs and requirements.

To customize severity levels, create a file named `m365advisor-config.json` in your `./tests/Custom` folder.
...
1. Run `Invoke-M365Advisor` and look at the test output
2. Check the [Tests Overview](/docs/tests/) documentation
3. Look at the test file source code in the M365Advisor repository

### Defining Severity Levels for Custom Tests

In addition to defining severity levels in `./tests/Custom/m365advisor-config.json`, you can also use the `-Tag` parameter in the `Describe` or `It` block of your Pester tests.
...
1. `./tests/Custom/m365advisor-config.json` (highest priority)
2. `-Tag` parameter in the test file
3. Main `./tests/m365advisor-config.json` (lowest priority)

:::warning Important
Avoid editing the `m365advisor-config.json` file in the `./tests` root folder. Any changes you make to this file will be overwritten when you update the M365Advisor tests.

Always use the `./tests/Custom/m365advisor-config.json` file for customizing severity levels.
:::
