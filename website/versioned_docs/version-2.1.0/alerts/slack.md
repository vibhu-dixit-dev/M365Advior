...
sidebar_position: 12
title: Set up M365Advisor Slack alerts
---

# Set up M365Advisor Slack Alerts

Your M365Advisor monitoring workflow can be configured to send a summary at the end of each monitoring cycle in Slack. This guide will walk you through the steps to set up Slack alerts in M365Advisor.
...
- Enter a name for your app, for example `M365Advisor DevOps` and select the Slack workspace where you want to post the notifications.
- Click "**Create App**".
...
```powershell
    ...
    - name: Notify Slack on success
      if: success()
      run: |
        curl -X POST -H 'Content-type: application/json' --data '{"text":"M365Advisor tests passed!"}' "${{ secrets.SLACK_WEBHOOK_URL }}"

    - name: Notify Slack on failure
      if: failure()
      run: |
        curl -X POST -H 'Content-type: application/json' --data '{"text":"M365Advisor tests failed!"}' "${{ secrets.SLACK_WEBHOOK_URL }}"
```

### Github Actions Slack example

You can use the output of the M365Advisor action to send a Slack notification. Here is an example of how to set up the workflow:
...
jobs:
  run-m365advisor-tests:
    ...
    - name: Run M365Advisor action
      id: m365advisor # this is important, by setting the id you can use the output of the action in the next steps
      uses: m365advisor365/m365advisor@main
      with:
    ...
    - name: Generate Slack Notification
      shell: pwsh
      if: always() && steps.m365advisor.outputs.results_json != ''
      run: |
        $results = Get-Content ${{ steps.m365advisor.outputs.results_json }} | ConvertFrom-Json
        ...
        $slackMessage = @{
          text = "M365Advisor Test Results $(if ($results.Result -eq 'Passed') { '✅' } else { '❌' })"
          attachments = @(
            @{
              color = if ($results.Result -eq 'Passed') { "good" } else { "danger" }
              text = $testSummary
