...
In this guide we will cover advanced concepts for writing custom tests with M365Advisor.

## Invoke-MtGraphRequest

M365Advisor provides a function called `Invoke-MtGraphRequest` that allows you to make direct calls to the Microsoft Graph API. This is an enhanced version of the `Invoke-MgGraphRequest` function that has been optimized for M365Advisor's use case to query Microsoft Graph data.
...
`Invoke-MtGraphRequest` has built-in caching to reduce the number of calls to the Microsoft Graph API when running M365Advisor tests.

This way you can write tests that call into any Graph API and if that data has already been fetched in the M365Advisor run, the cached data will be used instead of querying Microsoft Graph. This is one of the reasons we can run multiple tests in a very performant way.

The cache is reset when you run Invoke-M365Advisor to ensure you always have the latest data.
...
- Includes **ConsistencyLevel** by default to all the calls. This works for M365Advisor's read-only use case and allows you to use any of the advanced query filter options without worrying about the consistency flag.
