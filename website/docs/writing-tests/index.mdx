...
M365Advisor is built on top of [Pester](https://pester.dev) which uses an English-like format to define tests.

In this guide we will show you how to write custom tests using Pester and Microsoft Graph PowerShell.

## Pre-requisites

- You have set up M365Advisor following the [installation guide](/docs/installation).
- Successfully run **Invoke-M365Advisor** and tested with the out of the box tests.

## Creating the custom test file

In your M365Advisor folder you should have a folder called **Custom**. This is where you can create your custom test files.
...
It is important to use the .Tests.ps1 suffix in the file name so that your test file can be automatically discovered and tested when you run **Invoke-M365Advisor**.
...
Describe "ContosoEntraConfig" -Tag "Privilege", "Contoso" {
    It "CT0001: Check 'Contoso MFA Users' group" -Tag 'Severity:Medium' {
...
- Provide a [Severity level](../configuration/severity-levels.md) in the -Tag for the test. The available severity levels are **Critical**, **High**, **Medium**, **Low** and **Info**.
- Save the file.

## Running the custom test

To run the test, open PowerShell and navigate to the M365Advisor folder.

Run the following command to execute the test you just created.

```powershell
Invoke-M365Advisor .\tests\Custom\
```

You should now see the output of your test.

**Congratulations. You've just created your first M365Advisor test!**
...
If you want inspiration for custom tests, check out [this repository](https://devclate.github.io/Custom-M365Advisor-Tests/) by Clayton Tyger.
