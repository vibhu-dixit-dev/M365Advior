...
sidebar_position: 10
title: Set up M365Advisor email alerts
---

# Set up M365Advisor Email Alerts

Your M365Advisor monitoring workflow can be configured to send an email summary at the end of each monitoring cycle. This guide will walk you through the steps to set up email alerts in M365Advisor.
...
# Find your M365Advisor service principal. Update filter with the name of your app registration or managed identity
$entraSP = Get-MgServicePrincipal -Filter "DisplayName eq 'M365Advisor'"

# Register M365Advisor service principal in Exchange
...
$mailbox = Get-Mailbox m365advisordemo@contoso.onmicrosoft.com
# Or create a new shared mailbox:
# $mailbox = New-Mailbox -Shared -Name "M365Advisor" -PrimarySmtpAddress m365advisor@contoso.onmicrosoft.com

# Create a RBAC scope to reference the mailbox
New-ManagementScope -Name "rbac_M365Advisor" -RecipientRestrictionFilter "GUID -eq '$($mailbox.GUID)'"

# Assign Mail.Send role limited to only the M365Advisor scope/mailbox
New-ManagementRoleAssignment -App $entraSP.AppId -Role "Application Mail.Send" -CustomResourceScope "rbac_M365Advisor" -Name "M365Advisor Send Mail RBAC"
...
Write-Host "Use '$($mailbox.ExternalDirectoryObjectId)' when calling Invoke-M365Advisor -MailUserId or Send-MtMail -UserId"
```
...
- Search for the application you created to run as the `M365Advisor DevOps Account`.
...
Update your GitHub/Azure DevOps daily monitoring workflow to send the email alert using `Send-MtMail` after the M365Advisor tests have been run.
...
## Adding a link to detailed M365Advisor results in the email

The Send-MtMail cmdlet has a `-TestResultsUri` parameter that can be used to include a link to the detailed M365Advisor results in the email.
