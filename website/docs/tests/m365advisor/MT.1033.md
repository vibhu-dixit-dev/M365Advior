---
title: "MT.1033 - MT.1033.$($RegularUsersList.IndexOf($_)): User should be blocked from using legacy authentication ($($_.userPrincipalName))"
description: "Checks if the Conditional Access Policies for blocking legacy authentication is active and used. See Block legacy authentication - Microsoft Learn"
slug: /tests/MT.1033
className: generated-test-doc
sidebar_class_name: hidden
hide_table_of_contents: true
keywords:
  - "M365 Advisor"
  - "Microsoft 365 security"
  - "MT.1033"
  - "Unknown"
  - "CA"
  - "CAWhatIf"
  - "LongRunning"
  - "M365Advisor"
---

<!-- This file is generated by website/scripts/generate-test-docs.mjs. Do not edit manually. -->

# MT.1033 - MT.1033.$($RegularUsersList.IndexOf($_)): User should be blocked from using legacy authentication ($($_.userPrincipalName))

## Overview

Checks if the Conditional Access Policies for blocking legacy authentication is active and used.

See [Block legacy authentication - Microsoft Learn](https://learn.microsoft.com/entra/identity/conditional-access/howto-conditional-access-policy-block-legacy)

## Test Metadata

| Field | Value |
| --- | --- |
| Test ID | MT.1033 |
| Severity | Unknown |
| Suite | M365 Advisor |
| Category | CA |
| PowerShell test | [Test-MtCaWIFBlockLegacyAuthentication](/docs/commands/Test-MtCaWIFBlockLegacyAuthentication) |
| Tags | CA, CAWhatIf, LongRunning, M365Advisor, MT.1033 |

## Source

- Pester test: `tests\M365Advisor\Entra\Test-ConditionalAccessWhatIf.Tests.ps1`
- PowerShell source: `powershell\public\M365Advisor\entra\Test-MtCaWIFBlockLegacyAuthentication.ps1`
