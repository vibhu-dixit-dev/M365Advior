...
# <IIcon icon="mdi:gitlab" height="48" /> Set up M365Advisor in GitLab

This guide will walk you through setting up M365Advisor in GitLab and automate the running of tests using GitLab Pipelines (jobs).
...
## Set up your M365Advisor tests project (group) in GitLab
...
<!--
### 2. Create a new project and import the M365Advisor Tests repository, to keep updated yourself
 - private-tests = ToDo
...
4. ... https://github.com/m365advisor365/m365advisor-tests...
...
- Fill in the following fields
  ...
  - **Value**: `project_path:<mygroup>/<myproject>:ref_type:branch:ref:<branch>` E.g. `project_path:m365advisor/m365advisor-tests:ref_type:branch:ref:main`
...
- Open your `m365advisor-tests` GitLab project and go to **Settings**
  ...
  script:
    - |
      pwsh -Command '
        ...
        $M365AdvisorParameters.Add("DisableTelemetry", $false )
        Write-Host "Disable pester telemetry set to: $($M365AdvisorParameters.DisableTelemetry)"
        ...
        #$results = Invoke-M365Advisor -Path public-tests -PesterConfiguration $PesterConfiguration -OutputFolder test-results -OutputFolderFileName "test-results" -PassThru
...
- Open your `m365advisor-tests` GitLab project and go to **Build**
...
- Search a artifact to view the results e.g. `run_m365advisor_tests_*`
