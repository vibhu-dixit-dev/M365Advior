...
# <IIcon icon="devicon:azure" height="48" /> Setup M365Advisor in Azure Container App Jobs

This guide will summarize the major requirements in setting up M365Advisor in Azure Container App Jobs. You may find gaps in this guide or areas requiring clarification. Please [open a discussion](https://github.com/m365advisor365/m365advisor/discussions/new/choose) if you run into challenges this guide does not address.
...
- Update any M365Advisor tests each time the container is run
...
### main.ps1
$applicationId = "<Application (Client) ID>"
$tenantId = "<Tenant ID you want to run M365Advisor against>"
$applicationDisplayName = "<Application Display Name"
$keyVaultName = "<Key Vault Name>"
...
Get-AzStorageBlobContent -Container $stContainer.Name -Context $stAccount.Context -Blob $compare.Name -Destination /m365advisor/test-results/|Out-Null
$comparison = Compare-MtTestResult -BaseDir /m365advisor/test-results
$comparisonFile = New-Item -Path /m365advisor/ -Name "compare-$($compare.Name)" -Value $($comparison|ConvertTo-Json)
...
SHELL ["pwsh","-Command"]
COPY main.ps1 /
RUN New-Item /m365advisor -ItemType Directory
WORKDIR "/m365advisor"
...
```bash
sudo docker build -t m365advisorjob .
sudo docker tag m365advisorjob <yourRegistry>.azurecr.io/m365advisorjob
sudo pwsh -command "Connect-AzAccount -Identity"
sudo pwsh -command "Connect-AzContainerRegistry -Name <yourRegistry>"
sudo docker push <yourRegistry>.azurecr.io/m365advisorjob
```
