...
- You must have the **Global Administrator** OR **Privileged Role Administrator** and **Application Administrator** role in your Entra tenant. This is so the necessary permissions can be consented to the Workload Identity that Azure DevOps will use and the M365Advisor WebApp.
:::info Important
Graph permissions for the M365Advisor workload identity
...
This section will guide you through the template required to deploy M365Advisor on Azure. Depending on your needs, this can be done locally or through CI/CD pipelines.
...
@minLength(2)
param webAppName string = 'app-m365advisor-${uniqueString(subscription().id)}'
...
resource appServicePlan 'Microsoft.Web/serverfarms@2024-04-01' = {
  name: 'asp-m365advisor-${environment}'
  location: location
  kind: 'linux'
  sku: {
      name: skuASPName
...
resource graphM365AdvisorApp 'Microsoft.Graph/applications@v1.0' = {
  uniqueName: 'app-m365advisor-${environment}'
  signInAudience: 'AzureADMyOrg'
  displayName: 'app-m365advisor-${environment}'
  web: {
    redirectUris: [
...
resource roleAssignment 'Microsoft.Authorization/roleAssignments@2020-10-01-preview' = {
  ...
  properties: {
    ...
    principalId: serviceConnectionM365AdvisorSp.id
    principalType: 'ServicePrincipal'
  }
}

resource serviceConnectionM365AdvisorApp 'Microsoft.Graph/applications@v1.0' = {
  uniqueName: 'sc-app-m365advisor-${environment}'
  signInAudience: 'AzureADMyOrg'
  displayName: 'sc-app-m365advisor-${environment}'
}
...
resource serviceConnectionRoleAssignment 'Microsoft.Graph/appRoleAssignedTo@v1.0' = [for appRole in __m365advisorAppRoles__: {
...
resource serviceConnectionM365AdvisorSp 'Microsoft.Graph/servicePrincipals@v1.0' = {
  appId: serviceConnectionM365AdvisorApp.appId
}
...
output applicationClientId string = serviceConnectionM365AdvisorApp.appId
output applicationRegistrationName string = serviceConnectionM365AdvisorSp.displayName
...
param certificateResource = {
  certificateReferenceName: 'm365advisor-prod'
  customHostName: 'm365advisor.domainName.domainSuffix'
...
param subnetResource = {
  ...
  vnetName: 'vnet-m365advisor-001'
  resourceGroupName: 'rg-m365advisor-infra'
}
```

## M365Advisor Deployment
:::note
...
$resourceGroupName = 'rg-m365advisor-prod'
$ADOOrganization = 'contosodevelopment'
$ADOProjectName = 'm365advisorproject'
$location = 'westeurope'
...
function New-ADOServiceConnection {
    ...
    $Description = "Federated Identity connection for M365Advisor"
...
$DeploymentInfo = New-AzResourceGroupDeployment -Name 'M365Advisor' -ResourceGroupName $resourceGroupName -TemplateParameterFile $bicepparameterFile
...
Select/Create a new Azure DevOps repository called "M365Advisor".<br></br>
...
Select the M365Advisor repository<br></br>
...
variables:
  ...
  ServiceConnection: sc-app-m365advisor-prod
  ## Web App information
  WebAppSubscriptionId: e687a125-dd45-41af-ac62-42fe38cba48a
  WebAppResourceGroup: rg-m365advisor-prod
  WebAppName: app-m365advisor-3kl6lixbgbk40
  ## Entra information
  ...
  - task: AzurePowerShell@5
    inputs:
      ...
      Inline: |
        ...
        Write-verbose "Installing M365Advisor tests from GitHub" -verbose
        ...
        # Invoke M365Advisor for HTML page
        Write-verbose "Running M365Advisor tests" -verbose
        Invoke-M365Advisor -OutputHtmlFile "$TempOutputFolder/index.html"

        # Create the zip file
        Write-verbose "Compressing M365Advisor results to a zip file for zip-deployment" -verbose
    ...
    displayName: 'Run M365Advisor tests and upload result to web app'
```

## Viewing the Azure Resources
We can see the resources located in the resource group.

![Screenshot of the M365Advisor Azure resources](assets/azure-devops-webapp-resourcegroup.png)

The schedule of the Azure DevOps trigger, which will trigger every 12th hour to upload new M365Advisor report to the Azure Web App.<br></br> [Adjust the schedule](https://learn.microsoft.com/en-us/azure/devops/pipelines/process/scheduled-triggers?view=azure-devops&tabs=yaml#cron-syntax) to suit your needs.
```yaml
schedules:
- cron: "0 0,12 * * *"
  displayName: every 12h
...
![Screenshot of the M365Advisor report WebApp](assets/azure-devops-webapp-results.png)
