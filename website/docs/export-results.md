...
M365Advisor supports exporting test results to CSV and Excel files. This is useful for sharing test results with others or for further analysis in a spreadsheet program.

## Exporting a markdown summary

To export a compact markdown summary that contains only the counters table, use `Invoke-M365Advisor` with `-OutputMarkdownSummaryFile`.

```powershell
Invoke-M365Advisor -OutputMarkdownSummaryFile "C:\path\to\results-summary.md"
```
...
```powershell
$results = Invoke-M365Advisor -PassThru
