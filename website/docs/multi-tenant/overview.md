...
M365Advisor supports running security tests across multiple Microsoft 365 tenants and viewing the results in a single HTML report.
...
1. Run M365Advisor separately for each tenant using its own service connection
2. Save the JSON results from each run
3. Merge and generate the report: `Merge-MtM365AdvisorResult -Path *.json | Get-MtHtmlReport | Out-File report.html`
