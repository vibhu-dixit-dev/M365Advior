...
- Enter a name for the application (e.g. `M365Advisor DevOps Account`)
- Select **Register**
...
> The Exchange Online permissions are necessary to support tests that validate [Exchange Online configurations](https://m365advisor.dev/docs/installation#installing-azure-and-exchange-online-modules), such as the [CISA tests](https://m365advisor.dev/docs/tests/cisa/exo).
...
> The Teams permissions are necessary to support tests that validate [Teams configurations](https://m365advisor.dev/docs/installation#installing-azure-exchange-online-and-teams-modules).
...
> The Azure RBAC permissions are necessary to support tests that validate [Azure configurations](https://m365advisor.dev/docs/installation#installing-azure-and-exchange-online-modules), such as the [CISA tests](https://m365advisor.dev/docs/tests/cisa/entra#:~:text=Test%2DMtCisaDiagnosticSettings).
...
> The SharePoint Online permissions are necessary to support tests that validate [SharePoint Online configurations](https://m365advisor.dev/docs/tests/cisa/spo), such as the CISA SharePoint baseline controls.
...
```powershell
Register-PnPEntraIDAppForInteractiveLogin -ApplicationName "M365Advisor PnP" -Tenant [yourtenant].onmicrosoft.com -SharePointDelegatePermissions "AllSites.FullControl"
```
...
- Output the **Client ID** you will need for `Connect-M365Advisor`

> **Note:** M365Advisor's SharePoint tests are read-only. `AllSites.FullControl` (delegated) is sufficient.
>
> **Important:** After registering the app, open a **new PowerShell session** before running `Connect-M365Advisor`, as the registration process loads PnP assemblies that can conflict with Microsoft Graph.

#### Option B — Reuse the existing M365Advisor app registration

If you prefer not to create a separate app, you can update the M365Advisor app registration you created earlier to also support PnP PowerShell interactive login.

**1. Add the localhost redirect URI**

- Open your M365Advisor app registration in the [Entra admin center](https://entra.microsoft.com)
...
You can retrieve the Client ID of your M365Advisor app from Graph without looking it up manually. First connect to Graph, then query for the app by its display name:

```powershell
# Connect to Graph first
Connect-M365Advisor -Service Graph

# Retrieve the Client ID of the M365Advisor app registration
$clientId = (Get-MgApplication -Filter "displayName eq 'M365Advisor DevOps Account'").AppId
```
...
Connect-M365Advisor -Service Graph,SharePointOnline -SharePointClientId $clientId
```

> **Important:** After registering the app, open a **new PowerShell session** before running `Connect-M365Advisor`, as PnP assemblies loaded during the session can conflict with Microsoft Graph.
...
**3. Connect using Connect-M365Advisor**

Pass the certificate thumbprint directly to `Connect-M365Advisor` using `-SharePointCertificateThumbprint`. `-TenantId` is required for thumbprint-based auth:
...
```powershell
Connect-M365Advisor -Service Graph,SharePointOnline -SharePointClientId "<Client ID>" -UseDeviceCode
```
...
4. Assign a security role with read access:
   ...
   - A **custom role** (e.g. `M365Advisor Security Reader`) with Organization-level **Read** on: **Agent** (`bot`), **Agent component** (`botcomponent`), **User** (`systemuser`), and **Connection Reference** (`connectionreference`)
5. Click **Create**

#### Configure M365Advisor

Add the environment URL to `m365advisor-config.json`:
