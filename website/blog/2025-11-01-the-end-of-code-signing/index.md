...
When we started with M365Advisor, we were in the luxurious position to use a code signing certificate for all published versions. This, sadly, ends today.
...
If you use M365Advisor in a CI/CD pipeline with managed workers, nothing will noticeably change. The workers don't persist the module information after running, it and the next run will go through the module installation like always.

But if you have M365Advisor installed on your own machine, PowerShell will throw the following error, calling out that the new version is no longer signed.

```powershell
Update-Module M365Advisor -AllowPrerelease
```

![Install-Package: The version '1.3.105' of the module 'M365Advisor' being installed is not catalog signed. Ensure that the version '1.3.105' of the module 'M365Advisor' has the catalog file 'M365Advisor.cat' and signed with the same publisher 'CN=Fabian Bader,
O=Fabian Bader, L=Hamburg, C=DE' as the previously-installed module 'M365Advisor' with version '1.3.0' under the directory 'C:\Users\Fabian\Documents\PowerShell\Modules\M365Advisor\1.3.0'. If you still want to install or update, use -SkipPublisherCheck parameter.](img/errormessage.png)

> Install-Package: The version '1.3.105' of the module 'M365Advisor' being installed is not catalog signed. Ensure that the version '1.3.105' of the module 'M365Advisor' has the catalog file 'M365Advisor.cat' and signed with the same publisher 'CN=Fabian Bader, O=Fabian Bader, L=Hamburg, C=DE' as the previously-installed module 'M365Advisor' with version '1.3.0' under the directory 'C:\Users\Fabian\Documents\PowerShell\Modules\M365Advisor\1.3.0'. If you still want to install or update, use -SkipPublisherCheck parameter.
