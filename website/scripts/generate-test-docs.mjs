...
function generateLauncherScripts(writes) {
  ...
  for (const fw of frameworks) {
    const psCmd = [
      ...
      `if ($_a365) { try { $_bom = New-Object System.Text.UTF8Encoding $true; foreach ($_f in @('Audit365.psm1','Audit365.psd1','OrcaClasses.ps1')) { $_fp = Join-Path $_a365 $_f; if (Test-Path $_fp) { [System.IO.File]::WriteAllText($_fp, [System.IO.File]::ReadAllText($_fp, [System.Text.Encoding]::UTF8), $_bom) } }; $_tmpl = Join-Path $_a365 'assets\\ReportTemplate.html'; if (-not (Test-Path (Split-Path $_tmpl))) { New-Item -ItemType Directory -Force -Path (Split-Path $_tmpl) | Out-Null }; (New-Object System.Net.WebClient).DownloadFile('https://raw.githubusercontent.com/vibhu-dixit-dev/M365Advior/main/powershell/assets/ReportTemplate.html', $_tmpl) } catch {} }`,
