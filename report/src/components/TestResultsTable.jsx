...
export default function TestResultsTable(props) {
  ...
  const getSortedData = useCallback((data) => {
    if (!sortColumn) {
      const nonSkipped = data.filter(item => item.Result !== "Skipped");
      const skipped = data.filter(item => item.Result === "Skipped");
    ...
    const nonSkipped = sorted.filter(item => item.Result !== "Skipped");
    const skipped = sorted.filter(item => item.Result === "Skipped");
    return [...nonSkipped, ...skipped];
