<#
  ...
  Example Usage:
      Invoke-MtTest -Path "tests/M365Advisor/Drift/MT1060Drift.tests.ps1"
  Setup:
...
# In production, this should be part of the M365Advisor module.
BeforeAll {
    ...
    if (-not (Get-Command -Name Compare-MtJsonObject -ErrorAction SilentlyContinue)) {
        Write-Verbose "Loading Compare-MtJsonObject function from public/m365advisor/drift/Compare-MtJsonObject.ps1"
        . "$PSScriptRoot/../../../powershell/public/m365advisor/drift/Compare-MtJsonObject.ps1"
    }
}
...
BeforeDiscovery {
    ...
    $driftRoot = $env:M365ADVISOR_FOLDER_DRIFT
    ...
    # Ensure the Compare-MtJsonObject function is available
    if (-not (Get-Command -Name Compare-MtJsonObject -ErrorAction SilentlyContinue)) {
        Write-Warning "Compare-MtJsonObject function missing, not the right version of M365Advisor?"
        return $null
    }
...
Describe "M365Advisor/Drift" -ForEach $driftFolders {
