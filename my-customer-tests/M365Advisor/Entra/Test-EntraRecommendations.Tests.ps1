...
Describe "M365Advisor/Entra" -Tag "M365Advisor", "Entra",  "Recommendation" -ForEach $EntraRecommendations {

    # Define the test name and Id for each Entra recommendation.
    $RecommendationId = $_.id
    It "MT.1024.$($RecommendationId -replace '^[^_]+_', ''): $($_.displayName). See https://m365advisor.dev/docs/tests/MT.1024" -Tag "MT.1024", "$($_.recommendationType)" {
        ...
        $recommendationLinkMd = "`n`n➡️ Open [Recommendation - $($_.displayName)]($recommendationUrl) in the Entra admin portal.`n`n*Note: If the recommendation is not applicable for your tenant, it can be marked as **Dismissed** for M365Advisor to skip it in the future.*"
