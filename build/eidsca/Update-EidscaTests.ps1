<#
 .Synopsis
  Generates M365Advisor tests for the Entra ID Security Config Analyzer defined at https://github.com/Cloud-Architekt/AzureAD-Attack-Defense
  ...
  * Generates M365Advisor tests for each test defined in the JSON file
...
function UpdateTemplate($template, $control, $controlItem, $docName, $isDoc) {
    ...
    if ($currentValue -eq '' -or $control.ControlName -eq '') {
        ...
        # Map severity to M365Advisor values
...
$aadsc = ($aadsc | Where-Object { $_.CollectedBy -eq "M365Advisor" }).ControlArea
...
Describe "EIDSCA" -Tag "EIDSCA",  "%CheckId%" {
    It "%CheckId%: %ControlName% - %DisplayName%. See https://m365advisor.dev/docs/tests/%DocName%"%TestCases% {
        <#
