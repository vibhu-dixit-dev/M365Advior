...
<#
.SYNOPSIS
    Updates the tags documentation file with an inventory of tags used in M365Advisor tests.

.DESCRIPTION
    This script scans M365Advisor's tests directory for tags used in Pester tests and generates a
...
.PARAMETER TestsPath
    The path to the M365Advisor tests directory. Defaults to 'tests' within the repository root.
...
.EXAMPLE
    .\Update-TagsDocumentation.ps1 -RepoRoot 'C:\M365Advisor' -TestsPath 'C:\M365Advisor\tests' -TagsDocPath 'C:\M365Advisor\website\docs\tests\tags\readme.md'
...
$TagGroups = [ordered]@{
    ...
    'M365Advisor'   = { param($t) $t.Tag -match '^(MT\.|M365Advisor)' }
    'Ungrouped' = { param($t) $t.Tag -notmatch '^(CIS|L1|L2|CISA|MS\.|EIDSCA|ORCA|MT\.|M365Advisor)' }
}
...
Tags are used by M365Advisor to identify and group related tests. They can also be used to select specific tests to run or exclude during test execution. This makes them very useful, but they can also get in the way if too many tags are created. Our goal is to minimize the "signal to noise" ratio when it comes to tags by focusing on a few key areas:

- **Test Suites**: We use standardized tag categories for test suites that align with well-known benchmarks and baselines. This helps users quickly identify tests that align with these widely recognized standards or with M365Advisor's own suite of tests:
  ...
  - **M365Advisor**: Tags prefixed with `M365Advisor` or `MT` (e.g., `MT.1001`, `MT.1024`)
