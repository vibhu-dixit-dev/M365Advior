# M365Advisor Module Optimization Plan

This document describes a set of targeted build-time and structural optimizations for the
`m365advisor365/m365advisor` repository. The goal is to reduce `Import-Module` time and reduce the
...
```powershell
function Invoke-M365AdvisorBuild {
    [CmdletBinding()]
    param (
        [string] $OutputPath
    )
...
`powershell/internal/`, `powershell/core/`, `build/`, `tests/`, and `m365advisor-tests/`.
...
**Problem:** The current `M365Advisor.psm1` dot-sources hundreds of individual `.ps1` files at
import time using `Get-ChildItem` loops. Each file is a separate filesystem read. On slower
storage or cold-cache environments this is the dominant cost of `Import-Module M365Advisor`.

**Action:**

- During the build, concatenate all `.ps1` files from `powershell/internal/` and
  `powershell/public/` into a single `M365Advisor.psm1` in the output directory.
...
**Problem:** The `FunctionsToExport` array in `M365Advisor.psd1` is already explicitly populated
...
- Overwrite the `FunctionsToExport` array in the output `M365Advisor.psd1` with the extracted
  list.
...
- Update `ScriptsToProcess` in the output `M365Advisor.psd1` to reference only the single
  consolidated file.
...
**Note:** Do not consolidate the ORCA class files into the main `M365Advisor.psm1`. They must
...
- During the build, copy the `tests/` tree to `module/m365advisor-tests/` as-is.
...
**Impact on `Update-M365AdvisorTests`:**
No output-structure change is required for `Update-M365AdvisorTests`. It should continue to
...
- When set, the build script measures `Import-Module` time and `Invoke-M365Advisor` discovery
  time for both the pre-optimization and post-optimization output, and writes a summary.

```powershell
$PreOptimization = Measure-Command { Import-Module ./output/pre/M365Advisor.psd1 -Force }
$PostOptimization = Measure-Command { Import-Module ./output/post/M365Advisor.psd1 -Force }
...
**Location:** `./build/Build-M365AdvisorModule.ps1`
...
3. Consolidate `powershell/internal/` and `powershell/public/` `.ps1` files into
   `./module/M365Advisor.psm1` (Item 2).
4. Consolidate ORCA class files into `./module/OrcaClasses.ps1` in their required
   inheritance order (Item 4).
5. Copy and update `M365Advisor.psd1` to `./module/M365Advisor.psd1`, writing the auto-generated
   `FunctionsToExport` list and updating `ScriptsToProcess` to reference `OrcaClasses.ps1`
   (Items 3 and 4).
6. Copy the `tests/` tree to `./module/m365advisor-tests/` without modifying Pester file
   boundaries (Item 5).
7. Copy static assets (`assets/`, `M365Advisor.Format.ps1xml`, `README.md`) to `./module/`.
...
```text
./module/
├── M365Advisor.psd1               # updated manifest (auto-generated FunctionsToExport)
├── M365Advisor.psm1               # consolidated internal + public functions
├── M365Advisor.Format.ps1xml      # copied unchanged
├── OrcaClasses.ps1            # consolidated ORCA class definitions
├── assets/                    # copied unchanged
├── README.md                  # copied unchanged
└── m365advisor-tests/             # copied from tests/ with file boundaries preserved
    ├── CISA/
    ├── CIS/
    ├── EIDSCA/
    ├── M365Advisor/
...
`M365Advisor.psm1`, making code review noisy and creating the possibility of merge conflicts in
...
- Replace any inline consolidation or copy steps with a single call to
  `./build/Build-M365AdvisorModule.ps1`.
- Add a step that validates the output `./module/` directory contains the expected files
  before proceeding.
- Add a step that imports `./module/M365Advisor.psd1` and runs `Get-Help` against a
...
- Add a step that runs a smoke test by importing the module and calling
  `Get-Command -Module M365Advisor` to confirm the expected function count is exported.
...
- Ensure the publish step runs only after the build step has completed and all validation
  steps have passed.
- Confirm that the `ModuleVersion` in `./module/M365Advisor.psd1` has been correctly stamped by
...
- Both workflows should check out the repository and invoke `./build/Build-M365AdvisorModule.ps1`
...
- Run `./build/Build-M365AdvisorModule.ps1` to produce the publishable module in `./module/`.
...
- The `FunctionsToExport` list in `M365Advisor.psd1` is auto-generated at build time. Do not
  edit it manually.
- Pester test files in `./module/m365advisor-tests/` are copied from `./tests/` at build time.
  Do not edit build output manually.

### `./README.md`

- Confirm the user-facing installation instructions (`Install-Module`, `Install-M365AdvisorTests`,
  `Invoke-M365Advisor`) are unchanged. No user-facing change is introduced by this work.
- Add or update a "Building from source" section pointing to the website docs and noting
  that `./build/Build-M365AdvisorModule.ps1` is the entry point.
...
```
## M365Advisor Module Optimization
...
- [ ] #N  Item 7 — Create Build-M365AdvisorModule.ps1
...
| --- | --- |
| Remove `Microsoft.Graph.Authentication` from `RequiredModules` | `Connect-M365Advisor` is optional. Automation users never call it. 180+ functions call Graph cmdlets directly and would fail with cryptic errors if the module is not loaded. The performance gain is near zero when the module is already in the session, which covers virtually all real-world usage. |
...
1. Move comment-based help inside function bodies   (one-time source pass, own PR)
        ↓
2. Consolidate internal + public PS1 files into M365Advisor.psm1
...
5. Preserve Pester test file boundaries
        ↓
7. Write Build-M365AdvisorModule.ps1 in ./build/        (wraps 2-5 into one script)
        ↓
