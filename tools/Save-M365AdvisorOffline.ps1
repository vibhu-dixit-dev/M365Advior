function Save-M365AdvisorOffline {
    <#
    .SYNOPSIS
    Download local copies of M365Advisor and its dependencies for use on systems that cannot access the PowerShell Gallery.

    .DESCRIPTION
    This function downloads the M365Advisor module and its dependencies to a specified directory, allowing for offline
    ...
    .EXAMPLE
    Save-M365AdvisorOffline -DestinationPath ~/Downloads/M365Advisor
    ...
    if ($CreateZip.IsPresent -and $InstalledModules.Count -gt 0) {
        $ZipPath = Join-Path -Path $DestinationPath -ChildPath 'M365AdvisorModuleWithDependencies.zip'
        try {
