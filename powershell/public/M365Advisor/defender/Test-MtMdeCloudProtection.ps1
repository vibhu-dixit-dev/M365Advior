function Test-MtMdeCloudProtection {
    ...
    .LINK
        https://m365advisor.dev/docs/commands/Test-MtMdeCloudProtection
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    try {
        ...
        if ($testResult) {
            $testResultMarkdown = "Well done. Cloud protection is enabled in all $($policyConfig.TotalCount) assigned Defender Antivirus policies."
        } else {
