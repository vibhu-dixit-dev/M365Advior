function Test-MtMdeBehaviorMonitoring {
    ...
    .LINK
        https://m365advisor.dev/docs/commands/Test-MtMdeBehaviorMonitoring
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    Write-Verbose "Running Test-MtMdeBehaviorMonitoring..."
    ...
    try {
        ...
        if ($testResult) {
            $testResultMarkdown = "Well done. Behavior monitoring is enabled in all $($policyConfig.TotalCount) assigned Defender Antivirus policies."
        } else {
            $testResultMarkdown = "Behavior monitoring is not properly configured in all policies."
