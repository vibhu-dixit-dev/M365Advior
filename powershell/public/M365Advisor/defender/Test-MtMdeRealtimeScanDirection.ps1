function Test-MtMdeRealtimeScanDirection {
    ...
    .LINK
        https://m365advisor.dev/docs/commands/Test-MtMdeRealtimeScanDirection
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    Write-Verbose "Running Test-MtMdeRealtimeScanDirection..."
    ...
    try {
        ...
        if ($testResult) {
            $testResultMarkdown = "Well done. Real-time scan direction is correctly configured in all $($policyConfig.TotalCount) assigned Defender Antivirus policies."
        } else {
