...
.LINK
    https://m365advisor.dev/docs/commands/Test-MtAIAgentHardCodedCredentials
#>
...
function Test-MtAIAgentHardCodedCredentials {
    ...
    if ($null -eq $agents) {
        Add-MtTestResultDetail -SkippedBecause 'Custom' -SkippedCustomReason 'No Copilot Studio agent data available. Ensure DataverseEnvironmentUrl is configured in m365advisor-config.json and Connect-M365Advisor -Service Dataverse has been run. See https://m365advisor.dev/docs/tests/MT.1119 for prerequisites.'
        return $null
    }
