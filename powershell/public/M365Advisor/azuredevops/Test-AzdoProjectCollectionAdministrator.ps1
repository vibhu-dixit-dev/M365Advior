...
.LINK
    https://m365advisor.dev/docs/commands/Test-AzdoProjectCollectionAdministrator
#>
function Test-AzdoProjectCollectionAdministrator {
    ...
    if ($UniqueUsersWithPCA.Count -ge 4) {
        ...
        $result = $true
        $resultMarkdown = "Less than 4 users/service accounts are directly assigned to the Project Collection Administrators role.`n`n%TestResult%"
    }
