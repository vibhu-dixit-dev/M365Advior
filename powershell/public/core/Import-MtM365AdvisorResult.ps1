function Import-MtM365AdvisorResult {
    <#
     .Synopsis
     ...
     .Description
        ...
        of single-tenant M365AdvisorResults objects. This is the standard way to load
     ...
     .LINK
        https://m365advisor.dev/docs/commands/Import-MtM365AdvisorResult
    #>
    [CmdletBinding()]
    [OutputType([object[]])]
    param(
    ...
    process {
        ...
        if ($allResults.Count -eq 0) {
            Write-Warning "No valid M365Advisor result files were loaded."
        } else {
            Write-Verbose "Loaded $($allResults.Count) M365Advisor result(s) in total."
        }
...
function Import-SingleResultFile {
    ...
    if ($data.PSObject.Properties.Name -contains 'Tenants' -and $null -ne $data.Tenants) {
        ...
        foreach ($tenant in $data.Tenants) {
            if (Test-M365AdvisorResultValid -Result $tenant -RequiredProperties $RequiredProperties -SourceFile $FilePath) {
                $ResultList.Add($tenant)
            }
        }
    } else {
        # Single-tenant format
        if (Test-M365AdvisorResultValid -Result $data -RequiredProperties $RequiredProperties -SourceFile $FilePath) {
            $ResultList.Add($data)
        }
    }
}

function Test-M365AdvisorResultValid {
    <#
    .Synopsis
