function Add-MtM365AdvisorAppFederatedCredential {
    <#
    .SYNOPSIS
    ...
    The GitHub repository name (without the organization). E.g. m365advisor-tests.
    ...
    .PARAMETER Name
    The name for the federated credential. Defaults to 'm365advisor-devops-<org>-<repo>'.
    ...
    .LINK
    https://m365advisor.dev/docs/commands/Add-MtM365AdvisorAppFederatedCredential
    #>
    ...
    try {
        ...
        $app = Get-MtM365AdvisorApp @params
        if (-not $app) {
            $errorId = if($Id) { $Id } else { $AppId }
            Write-Error "M365Advisor application not found with the specified identifier: $errorId."
            return
        }

        Write-Host "Adding federated credential to M365Advisor application" -ForegroundColor Green
        Write-Output $app

        if (-not $Name) { # Set default name if not provided
            $Name = "m365advisor-devops-$($GitHubOrganization)-$($GitHubRepository)"
        }
        ...
        $federatedCredential = @{
            ...
            description = "Federated credential for GitHub Actions in $GitHubOrganization/$GitHubRepository ($GitHubBranch branch) - Created by M365Advisor"
