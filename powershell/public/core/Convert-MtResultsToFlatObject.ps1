function Convert-MtResultsToFlatObject {
    ...
    .DESCRIPTION
    Convert M365Advisor test results to a flattened object that can be exported to CSV or Excel. This function exports
    ...
    Convert the M365Advisor test results in C:\path\to\results.json to a flattened object, and then export that object to an Excel file (C:\path\to\results.xlsx). Requires the ImportExcel module.
    ...
    .LINK
    https://m365advisor.dev/docs/commands/Convert-MtResultsToFlatObject
    ...
    param (
        # The M365Advisor test results passed from the pipeline using `Invoke-M365Advisor -PassThru` or as an input object.
        ...
        [Alias('M365AdvisorResults')]
        [ValidateNotNullOrEmpty()]
        [psobject] $InputObject,

        # The path to the JSON file containing the M365Advisor test results.
        [Parameter(Mandatory, Position = 0, HelpMessage = 'The path to the JSON file containing the M365Advisor test results.', ParameterSetName = 'FromFile')]
        [Parameter(Mandatory, Position = 0, HelpMessage = 'The path to the JSON file containing the M365Advisor test results.', ParameterSetName = 'CSV')]
        [Parameter(Mandatory, Position = 0, HelpMessage = 'The path to the JSON file containing the M365Advisor test results.', ParameterSetName = 'XLSX')]
        ...
        # The path to the CSV file to which the M365Advisor test results will be exported.
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the CSV file to which the M365Advisor test results will be exported.', ParameterSetName = 'CSV')]
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the CSV file to which the M365Advisor test results will be exported.', ParameterSetName = 'FromInputObject')]
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the CSV file to which the M365Advisor test results will be exported.', ParameterSetName = 'FromFile')]
        [string] $CsvFilePath = "$($JsonFilePath -replace '\.json$', '.csv')",

        # The path to the Excel file to which the M365Advisor test results will be exported.
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the Excel file to which the M365Advisor test results will be exported.', ParameterSetName = 'XLSX')]
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the Excel file to which the M365Advisor test results will be exported.', ParameterSetName = 'FromInputObject')]
        [Parameter(HelpMessage = 'Optional when JsonFilePath is provided. The path to the Excel file to which the M365Advisor test results will be exported.', ParameterSetName = 'FromFile')]
    ...
    process {
        ...
        if ($ExportCsv.IsPresent -or $PSBoundParameters.ContainsKey('CsvFilePath')) {
            ...
            try {
                ...
                Write-Verbose "Exported the M365Advisor test results to '$CsvFilePath'." -InformationAction Continue
            } catch {
                Write-Error "Failed to export the M365Advisor test results to a CSV file. $_"
            }
        }
        ...
        if ($ExportExcel.IsPresent -or $PSBoundParameters.ContainsKey('ExcelFilePath')) {
            ...
            try {
                ...
                Write-Verbose "Exported the M365Advisor test results to '$ExcelFilePath'." -InformationAction Continue
            } catch [System.Management.Automation.CommandNotFoundException] {
                Write-Error "The ImportExcel module is required to export the M365Advisor test results to an Excel file. Install the module using ``Import-Module -Name 'ImportExcel'`` and try again."

            } catch {
                Write-Error "Failed to export the M365Advisor test results to an Excel file. $_"
            }
        }
    }
