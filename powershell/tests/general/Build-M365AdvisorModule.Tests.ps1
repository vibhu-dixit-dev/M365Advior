BeforeAll {
    $script:BuildScriptPath = Resolve-Path "$PSScriptRoot/../../../build/Build-M365AdvisorModule.ps1"
    ...
    function Initialize-BuildFixture {
        ...
        $FixtureRoot = Join-Path $script:RepoRoot ".tmp-m365advisor-build-tests-$([guid]::NewGuid())"
...
'@
        Write-TestFileContent -Path (Join-Path $SourceRoot 'M365Advisor.Format.ps1xml') -Content @'
<Configuration>
  <ViewDefinitions>
  </ViewDefinitions>
</Configuration>
...
'@
        ...
        New-ModuleManifest `
            -Path (Join-Path $SourceRoot 'M365Advisor.psd1') `
            -RootModule 'M365Advisor.psm1' `
            -ModuleVersion '0.0.1' `
            -Guid ([guid]::NewGuid()) `
            -Author 'M365Advisor' `
            -CompanyName 'M365Advisor' `
            -AliasesToExport @('mt') `
...
AfterAll {
    Get-ChildItem -Path $script:RepoRoot -Directory -Filter '.tmp-m365advisor-build-tests-*' |
        Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
}

Describe 'Build-M365AdvisorModule' {
    ...
    Context 'When building a valid fixture' {
        It 'strips file-level preambles, recurses ORCA checks, and writes parseable outputs' {
            ...
            try {
                ...
                $OutputPsm1 = Join-Path $Fixture.OutputRoot 'M365Advisor.psm1'
                $OutputOrcaClasses = Join-Path $Fixture.OutputRoot 'OrcaClasses.ps1'
                $OutputManifest = Join-Path $Fixture.OutputRoot 'M365Advisor.psd1'
