BeforeDiscovery {
    ...
    $manifest = Import-PowerShellDataFile -Path (Join-Path -Path $moduleRoot -ChildPath 'M365Advisor.psd1')
}
...
Describe 'Validating the module manifest' -ForEach @{ moduleRoot = $moduleRoot; manifest = $manifest } {
    Context 'Basic resources validation' {
        ...
        It 'Manifest is valid' {
            Test-ModuleManifest -Path (Join-Path -Path $moduleRoot -ChildPath 'M365Advisor.psd1')
