Describe 'Import-MtM365AdvisorResult' {
    BeforeAll {
        # Create a temp directory for test JSON files
        $testDir = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "M365AdvisorImportTests_$([guid]::NewGuid().ToString('N'))"
    ...
    Context 'Loading single-tenant JSON files' {
        It 'Should load a single file by path' {
            $results = Import-MtM365AdvisorResult -Path $file1
        ...
        It 'Should load multiple files by path' {
            $results = Import-MtM365AdvisorResult -Path $file1, $file2
    ...
    Context 'Loading multi-tenant merged JSON' {
        It 'Should auto-expand a merged file into individual tenant results' {
            $results = Import-MtM365AdvisorResult -Path $mergedFile
    ...
    Context 'Directory discovery' {
        It 'Should discover TestResults-*.json files in a directory' {
            $results = Import-MtM365AdvisorResult -Path $subDir
        ...
        It 'Should fall back to all *.json files when no TestResults-* files exist' {
            ...
            $results = Import-MtM365AdvisorResult -Path $customDir
    ...
    Context 'Glob patterns' {
        It 'Should resolve glob patterns for file paths' {
            ...
            $results = Import-MtM365AdvisorResult -Path $globPattern
    ...
    Context 'Validation and error handling' {
        It 'Should skip files missing required properties with a warning' {
            $results = Import-MtM365AdvisorResult -Path $invalidFile -WarningAction SilentlyContinue

            $results.Count | Should -BeExactly 0
        }

        It 'Should skip malformed JSON files with a warning' {
            $results = Import-MtM365AdvisorResult -Path $malformedFile -WarningAction SilentlyContinue

            $results.Count | Should -BeExactly 0
        }

        It 'Should emit an error for nonexistent paths' {
            { Import-MtM365AdvisorResult -Path './nonexistent/path/file.json' -ErrorAction Stop } | Should -Throw
        }
    }

    Context 'Pipeline input' {
        It 'Should accept pipeline input from strings' {
            $results = @($file1, $file2) | Import-MtM365AdvisorResult
        ...
        It 'Should accept pipeline input from Get-ChildItem via FullName alias' {
            $results = Get-ChildItem -Path $testDir -Filter 'TestResults-*.json' | Import-MtM365AdvisorResult
    ...
    Context 'Integration with Merge-MtM365AdvisorResult' {
        It 'Should pipe directly into Merge-MtM365AdvisorResult' {
            $merged = Import-MtM365AdvisorResult -Path $file1, $file2 | Merge-MtM365AdvisorResult
