Describe 'Get-MtHtmlReport' {
    BeforeAll {
        ...
        $singleTenant = [PSCustomObject]@{
            ...
            LoadedModules  = @()
            InvokeCommand  = 'Invoke-M365Advisor'
            MgContext      = [PSCustomObject]@{ TenantId = 'single-tenant-id' }
            M365AdvisorConfig  = [PSCustomObject]@{
            ...
            Tests          = @(
                [PSCustomObject]@{
                    ...
                    Severity = 'High'; Tag = @('MT.1001'); Block = 'M365Advisor'
                    Duration = '00:00:01'; ErrorRecord = @()
                    ResultDetail = [PSCustomObject]@{ TestDescription = 'Desc'; TestResult = 'OK' }
                }
            )
            Blocks         = @(
                [PSCustomObject]@{ Name = 'M365Advisor'; PassedCount = 4; FailedCount = 1; TotalCount = 5 }
            )
            EndOfJson      = 'EndOfJson'
        }
        ...
        $tenant1 = [PSCustomObject]@{
            ...
            LatestVersion  = '2.0.0'
            Tests          = @(
                [PSCustomObject]@{ Index = 1; Id = 'MT.1001'; Result = 'Passed'; Block = 'M365Advisor' }
            )
            Blocks         = @(
                [PSCustomObject]@{ Name = 'M365Advisor'; PassedCount = 3; TotalCount = 3 }
            )
            EndOfJson      = 'EndOfJson'
        }
        ...
        $tenant2 = [PSCustomObject]@{
            ...
            LatestVersion  = '2.0.0'
            Tests          = @(
                [PSCustomObject]@{ Index = 1; Id = 'MT.1001'; Result = 'Failed'; Block = 'M365Advisor' }
            )
            Blocks         = @(
    ...
    Context 'Single-tenant report' {
        ...
        It 'Should generate valid HTML' {
            $html = Get-MtHtmlReport -M365AdvisorResults $singleTenant
    ...
    Context 'Multi-tenant report' {
        BeforeAll {
            $merged = Merge-MtM365AdvisorResult -M365AdvisorResults @($tenant1, $tenant2)
        }
