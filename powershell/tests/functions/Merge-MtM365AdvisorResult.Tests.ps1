Describe 'Merge-MtM365AdvisorResult' {
    BeforeAll {
        $tenant1 = [PSCustomObject]@{
            ...
            LatestVersion  = '2.0.0'
            Tests          = @(
                [PSCustomObject]@{ Id = 'MT.1001'; Result = 'Passed'; Block = 'M365Advisor' }
                [PSCustomObject]@{ Id = 'MT.1002'; Result = 'Failed'; Block = 'M365Advisor' }
            )
            Blocks         = @(
                [PSCustomObject]@{ Name = 'M365Advisor'; PassedCount = 8; FailedCount = 2; TotalCount = 10 }
            )
            EndOfJson      = 'EndOfJson'
        }
        ...
        $tenant2 = [PSCustomObject]@{
            ...
            LatestVersion  = '2.0.0'
            Tests          = @(
                [PSCustomObject]@{ Id = 'MT.1001'; Result = 'Failed'; Block = 'M365Advisor' }
                [PSCustomObject]@{ Id = 'MT.1002'; Result = 'Passed'; Block = 'M365Advisor' }
            )
            Blocks         = @(
                [PSCustomObject]@{ Name = 'M365Advisor'; PassedCount = 3; FailedCount = 2; TotalCount = 5 }
            )
            EndOfJson      = 'EndOfJson'
        }

        # Create temp directory with JSON files for -Path tests
        $testDir = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "M365AdvisorMergeTests_$([guid]::NewGuid().ToString('N'))"
    ...
    It 'Should throw when no results are provided' {
        { Merge-MtM365AdvisorResult -M365AdvisorResults @() } | Should -Throw
    }

    It 'Should wrap a single result in Tenants array' {
        $result = Merge-MtM365AdvisorResult -M365AdvisorResults @($tenant1)
    ...
    It 'Should merge two tenant results into Tenants array' {
        $result = Merge-MtM365AdvisorResult -M365AdvisorResults @($tenant1, $tenant2)
    ...
    It 'Should throw when a result is missing the Tests property' {
        ...
        { Merge-MtM365AdvisorResult -M365AdvisorResults @($invalid) } | Should -Throw '*Tests*'
    }
    ...
    It 'Should merge three or more tenant results' {
        $tenant3 = [PSCustomObject]@{
            ...
            Blocks         = @(
                [PSCustomObject]@{ Name = 'M365Advisor'; PassedCount = 7; TotalCount = 7 }
            )
            EndOfJson      = 'EndOfJson'
        }

        $result = Merge-MtM365AdvisorResult -M365AdvisorResults @($tenant1, $tenant2, $tenant3)
    ...
    It 'Should handle a result with empty TenantName' {
        ...
        $result = Merge-MtM365AdvisorResult -M365AdvisorResults @($noName)
    ...
    Context 'FromPath parameter set' {
        It 'Should merge results from file paths' {
            $result = Merge-MtM365AdvisorResult -Path $file1, $file2
        ...
        It 'Should merge results from a directory' {
            $result = Merge-MtM365AdvisorResult -Path $testDir
        ...
        It 'Should merge results from a glob pattern' {
            ...
            $result = Merge-MtM365AdvisorResult -Path $globPattern
        ...
        It 'Should throw when path contains no valid results' {
            ...
            { Merge-MtM365AdvisorResult -Path $emptyDir } | Should -Throw
        }
    ...
    Context 'Pipeline input' {
        It 'Should accept pipeline input from Import-MtM365AdvisorResult' {
            $result = Import-MtM365AdvisorResult -Path $file1, $file2 | Merge-MtM365AdvisorResult
        ...
        It 'Should accept pipeline input from in-memory objects' {
            $result = @($tenant1, $tenant2) | Merge-MtM365AdvisorResult
        ...
        It 'Should include all runs even when same TenantId appears multiple times' {
            ...
            $result = @($tenant1, $tenant1) | Merge-MtM365AdvisorResult
