Describe 'Invoke-M365Advisor' {
    ...
    It 'Validates smoke test results' {
        ...
        $Result = Invoke-M365Advisor @m365advisorParams
        # Dynamically calculate expected counts from smoke test files
        $smokeTestFiles = Get-ChildItem -Path $m365advisorParams.Path -Filter *.ps1
