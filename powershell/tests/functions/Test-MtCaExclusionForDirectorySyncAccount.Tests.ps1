Describe 'Test-MtCaExclusionForDirectorySyncAccount' {
    BeforeAll {
        ...
        Mock -ModuleName M365Advisor Get-MtRoleInfo {
            param($RoleName)
            switch ($RoleName) {
    ...
    Context 'No sync account members exist in the tenant' {
        ...
        BeforeEach {
            Mock -ModuleName M365Advisor Get-MtRoleMember { return $null }
        }

        It 'Should return true and mark test as not applicable' {
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy { return @() }
    ...
    Context 'Service principal-only members (ABA — Application-Based Authentication, Entra Connect v2.5.76.0+)' {
        ...
        BeforeEach {
            ...
            Mock -ModuleName M365Advisor Get-MtRoleMember { return $script:syncServicePrincipal }
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy {
    ...
    Context 'Policy does not target all applications' {
        ...
        BeforeEach {
            Mock -ModuleName M365Advisor Get-MtRoleMember { return $script:syncUser1 }
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy {
    ...
    Context 'Policy targets all users and all sync user members are individually excluded' {
        ...
        BeforeEach {
            Mock -ModuleName M365Advisor Get-MtRoleMember { return @($script:syncUser1, $script:syncUser2) }
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy {
    ...
    Context 'Policy targets all users and sync accounts are only partially excluded (service principal skipped, one user excluded)' {
        ...
        BeforeEach {
            ...
            Mock -ModuleName M365Advisor Get-MtRoleMember { return @($script:syncUser1, $script:syncServicePrincipal) }
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy {
                return @(New-CaPolicy -ExcludeUsers @($script:syncUserId1))
            }
        }
    ...
    Context 'Free Entra ID license' {
        ...
        It 'Should return null and skip the test' {
            Mock -ModuleName M365Advisor Get-MtLicenseInformation { return 'Free' }
    ...
    Context 'Get-MtRoleInfo returns null for both sync roles (module reload / $script:MtRoles uninitialised)' {
        ...
        BeforeEach {
            ...
            Mock -ModuleName M365Advisor Get-MtRoleInfo { return $null }
            Mock -ModuleName M365Advisor Get-MtRoleMember {}
            Mock -ModuleName M365Advisor Get-MtConditionalAccessPolicy { return @() }
        }
        ...
        It 'Should not call Get-MtRoleMember when role info is null' {
            ...
            Should -Invoke Get-MtRoleMember -ModuleName M365Advisor -Times 0 -Exactly
        }
    }
