Describe 'Get-MailAuthenticationRecord' {
    BeforeEach {
        ...
        Mock ConvertFrom-MailAuthenticationRecordSpf -ModuleName M365Advisor {
            [PSCustomObject]@{
                terms = @()
            }
        }

        Mock Resolve-SPFRecord -ModuleName M365Advisor {
            [PSCustomObject]@{
                IPAddress = '192.0.2.1'
            }
        }
    ...
    It 'Does not pass Server to SPF lookup when DnsServerIpAddress is omitted' {
        ...
        Should -Invoke Resolve-SPFRecord -ModuleName M365Advisor -Exactly 1 -ParameterFilter {
