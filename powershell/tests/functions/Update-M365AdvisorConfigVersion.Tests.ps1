BeforeAll {
    $script:BuildScriptPath = Resolve-Path "$PSScriptRoot/../../../build/Update-M365AdvisorConfigVersion.ps1"
...
Describe 'Update-M365AdvisorConfigVersion' {
    It 'updates existing ModuleVersion and ConfigVersion fields' {
        $configPath = Join-Path 'TestDrive:' 'm365advisor-config.json'
...
'@
    ...
    It 'preserves an explicitly provided empty ConfigVersion' {
        $configPath = Join-Path 'TestDrive:' 'm365advisor-config-empty-version.json'
        Write-TestConfig -Path $configPath -Content @'
{
  "ModuleVersion": "1.0.0",
...
'@
    ...
    It 'computes ConfigVersion from git commit dates in UTC when ConfigVersion is omitted' {
        ...
        $configPath = Join-Path $repoPath 'm365advisor-config.json'
        ...
        & git -C $repoProviderPath config user.name 'M365Advisor Test'
        & git -C $repoProviderPath config user.email 'm365advisor-test@example.com'
...
'@
        ...
        & git -C $repoProviderPath add m365advisor-config.json
...
'@
        ...
        $configPath = Join-Path 'TestDrive:' "m365advisor-config-missing-$MissingField.json"
