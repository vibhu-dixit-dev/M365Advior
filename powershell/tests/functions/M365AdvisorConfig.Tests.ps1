Describe 'M365Advisor Configuration File - tests/m365advisor-config.json' {
    Context 'Version fields' {
        It 'has a top-level ModuleVersion string' {
            $repoRoot = Resolve-Path (Join-Path $PSScriptRoot '../../..')
            $configPath = Join-Path $repoRoot 'tests/m365advisor-config.json'
        ...
        It 'source ModuleVersion matches powershell/M365Advisor.psd1 ModuleVersion' {
            # The published artifact's ModuleVersion is stamped by CI, but the
            # source-tree value should track M365Advisor.psd1 so a clone shows a
            # sensible number. Drift between the two is a maintenance bug.
            $repoRoot = Resolve-Path (Join-Path $PSScriptRoot '../../..')
            $configPath = Join-Path $repoRoot 'tests/m365advisor-config.json'
            $manifestPath = Join-Path $repoRoot 'powershell/M365Advisor.psd1'

            $configJson = Get-Content -Path $configPath -Raw | ConvertFrom-Json
            $manifest = Import-PowerShellDataFile -Path $manifestPath

            $configJson.ModuleVersion | Should -Be $manifest.ModuleVersion -Because "tests/m365advisor-config.json ModuleVersion ($($configJson.ModuleVersion)) should match powershell/M365Advisor.psd1 ModuleVersion ($($manifest.ModuleVersion))"
        }
    ...
    Context 'TestSettings array' {
        It 'should be sorted by Id' {
            ...
            if ($null -ne $testSettings -and $testSettings.Count -gt 1) {
                ...
                $isSorted | Should -BeTrue -Because "TestId: Expected='$expected' Actual: '$actual' ❌ is not sorted in m365advisor-config.json. The Test IDs should be sorted for better diffs, history, and easier maintenance."
