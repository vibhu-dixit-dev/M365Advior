...
.DESCRIPTION
    ...
    This function is called by Connect-M365Advisor as a fallback when
    DataverseEnvironmentUrl is not explicitly configured in m365advisor-config.json.
...
function Get-MtDataverseEnvironmentUrl {
    ...
    if (-not $azContext) {
        Write-Warning "No active Azure context. Ensure you are connected via 'Connect-M365Advisor -Service Dataverse'."
        return $null
    }
    ...
    try {
        ...
        Write-Warning "Failed to get Global Discovery Service token. Ensure you are connected via 'Connect-M365Advisor -Service Dataverse'. Error: $_"
        return $null
    }
    ...
    if (-not $gdsResponse.value -or $gdsResponse.value.Count -eq 0) {
        Write-Warning "No Dataverse environments found via Global Discovery Service. If you have a GCC environment or a specific environment URL, configure 'DataverseEnvironmentUrl' in m365advisor-config.json GlobalSettings."
        return $null
    }
    ...
    if ($gdsResponse.value.Count -gt 1) {
        Write-Verbose "Found $($gdsResponse.value.Count) Dataverse environments. Using the first: '$($selectedEnv.FriendlyName)' ($discoveredUrl). To use a different environment, configure 'DataverseEnvironmentUrl' in m365advisor-config.json GlobalSettings."
    } else {
