...
.DESCRIPTION
    ...
    Dataverse connection established via Connect-M365Advisor -Service Dataverse.
    ...
    DataverseEnvironmentUrl in m365advisor-config.json). This function reads the
...
.LINK
    https://m365advisor.dev/docs/commands/Get-MtAIAgentInfo
#>
...
function Get-MtAIAgentInfo {
    ...
    # Read pre-resolved Dataverse connection details from session (set by Connect-M365Advisor)
    ...
    if ([string]::IsNullOrEmpty($apiBase) -or [string]::IsNullOrEmpty($resourceUrl)) {
        Write-Warning "Dataverse connection not established. Ensure you are connected via 'Connect-M365Advisor -Service Dataverse'."
    ...
    try {
        ...
        Write-Warning "Failed to get Dataverse access token for Copilot Studio. Ensure you are connected via 'Connect-M365Advisor -Service Dataverse'. Error: $_"
        $__MtSession.AIAgentInfo = @()
        return $null
    }
