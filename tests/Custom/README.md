...
- **Running Custom Tests**: To execute tests located in this `Custom` directory exclusively, use the `-Path` parameter with `Invoke-M365Advisor`. For example:

```powershell
Invoke-M365Advisor -Path ./Custom
```
