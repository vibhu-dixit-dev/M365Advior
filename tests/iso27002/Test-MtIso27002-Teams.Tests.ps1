##########################################################################
# ISO 27002 / ISO 27002 - Teams Security Tests
# Ported from M365-Assess: Collaboration/Get-TeamsSecurityConfig.ps1
# Controls: A.5.14, A.5.23, A.8.4, A.8.20
##########################################################################

Describe "ISO 27002 - Teams External Access" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.5.23", "ISO27002:A.5.14", "ISO27002:A.5.23", "ISO27002:A.5.14" {
    BeforeAll {
        try {
    ...
    It "ISO27002.5.23.1: Communication with unmanaged Teams users (personal accounts) must be disabled" {
    ...
    It "ISO27002.5.23.2: External unmanaged Teams users must not be able to initiate conversations" {
        if (-not $script:teamsClientConfig) {
    ...
    It "ISO27002.5.14.8: External Teams domain access must be disabled or restricted to specific domains" {
        if (-not $script:teamsClientConfig) {
            Set-ItResult -Skipped -Because "Teams client config endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.5.23.3: Skype for Business / consumer interop must be disabled" {
        if (-not $script:teamsClientConfig) {
            Set-ItResult -Skipped -Because "Teams client config endpoint not available in this cloud environment"
            return
        }
...
Describe "ISO 27002 - Teams Client Configuration" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.20", "ISO27002:A.5.14", "ISO27002:A.8.20", "ISO27002:A.5.14" {
    BeforeAll {
        try {
    ...
    It "ISO27002.8.20.4: Third-party cloud storage providers must be disabled in Teams" {
    ...
    It "ISO27002.8.20.5: Email-into-Channel must be disabled" {
        if (-not $script:teamsClientConfig2) {
            Set-ItResult -Skipped -Because "Teams client config endpoint not available in this cloud environment"
            return
        }
...
Describe "ISO 27002 - Teams Meeting Policies" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.5.14", "ISO27002:A.8.4", "ISO27002:A.5.14", "ISO27002:A.8.4" {
    BeforeAll {
        try {
    ...
    It "ISO27002.5.14.9: Anonymous users must not be able to join meetings" {
    ...
    It "ISO27002.5.14.10: Anonymous users must not be able to start meetings" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.8.4.3: Lobby bypass must be restricted to org members only (not everyone)" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.8.4.4: Dial-in users must not bypass the meeting lobby" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.8.4.5: External participants must not be able to give or request control" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.5.14.11: Anonymous users must not be able to use meeting chat" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.8.4.6: Default meeting presenter role must be OrganizerOnly" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.5.14.12: External non-trusted users must not be able to use meeting chat" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
    ...
    It "ISO27002.5.14.13: Cloud recording must be disabled by default" {
        if (-not $script:meetingPolicy) {
            Set-ItResult -Skipped -Because "Teams meeting policy endpoint not available in this cloud environment"
            return
        }
