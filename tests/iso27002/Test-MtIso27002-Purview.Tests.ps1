##########################################################################
# ISO 27002 / ISO 27002 - Microsoft Purview Security & Compliance Tests
# Ported from M365-Assess: Purview/* Checks
# Controls: A.8.10, A.8.15
##########################################################################

Describe "ISO 27002 - Purview Data Retention Policies" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.10", "ISO27002:A.8.10" {
    BeforeAll {
    ...
    It "ISO27002.8.10.1: At least one data retention policy must be configured and active" {
    ...
    It "ISO27002.8.10.2: Exchange locations must be covered by a retention policy" {
        if (-not $script:retentionCmdAvailable -or $null -eq $script:policies) {
            Set-ItResult -Skipped -Because "Purview connection not active or Get-RetentionCompliancePolicy is not accessible"
            return
        }
    ...
    It "ISO27002.8.10.3: Teams channel locations and chats must be covered by a retention policy" {
        if (-not $script:retentionCmdAvailable -or $null -eq $script:policies) {
            Set-ItResult -Skipped -Because "Purview connection not active or Get-RetentionCompliancePolicy is not accessible"
            return
        }
    ...
    It "ISO27002.8.10.4: SharePoint and OneDrive locations must be covered by a retention policy" {
        if (-not $script:retentionCmdAvailable -or $null -eq $script:policies) {
            Set-ItResult -Skipped -Because "Purview connection not active or Get-RetentionCompliancePolicy is not accessible"
            return
        }
    ...
    It "ISO27002.8.10.5: Enabled retention policies must be in Enforce mode (not simulation/test)" {
        if (-not $script:retentionCmdAvailable -or $null -eq $script:policies) {
            Set-ItResult -Skipped -Because "Purview connection not active or Get-RetentionCompliancePolicy is not accessible"
            return
        }
...
Describe "ISO 27002 - Purview Audit & Logging" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.15", "ISO27002:A.8.15" {
    BeforeAll {
    ...
    It "ISO27002.8.15.4: Unified Audit Log Ingestion must be enabled" {
    ...
    It "ISO27002.8.15.5: Administrative auditing must be enabled globally" {
        if (-not $script:auditCmdAvailable -or $null -eq $script:auditConfig) {
            Set-ItResult -Skipped -Because "Exchange Online / Get-AdminAuditLogConfig is not accessible"
            return
        }
