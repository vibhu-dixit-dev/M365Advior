##########################################################################
# ISO 27001 / ISO 27002 - Intune / Endpoint Security Tests
# Ported from M365-Assess: Intune/Get-IntuneSecurityConfig.ps1
# Controls: A.8.9, A.8.19
##########################################################################

Describe "ISO 27001 - Intune Device Management" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.9", "ISO27001:A.8.19", "ISO27002:A.8.9", "ISO27002:A.8.19" {
    BeforeAll {
    ...
    It "ISO27001.A.8.9.8: Non-compliant device threshold must be 30 days or less" {
    ...
    It "ISO27001.A.8.9.9: Personal device enrollment must be blocked on all platforms (iOS, Android, Windows)" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
...
Describe "ISO 27001 - Intune Device Compliance Policies" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.9", "ISO27002:A.8.9" {
    It "ISO27001.A.8.9.10: At least one device compliance policy must be configured" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27001.A.8.9.11: At least one device configuration profile must exist for security hardening" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27001.A.8.19.1: Windows Autopilot or device enrollment program must be configured" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
