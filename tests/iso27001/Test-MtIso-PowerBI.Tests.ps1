##########################################################################
# ISO 27001 / ISO 27002 - Power BI Security Tests
# Ported from M365-Assess: PowerBI/Get-PowerBISecurityConfig.ps1
# Controls: A.5.15, A.5.16, A.8.12, A.8.13, A.8.20, A.8.24
##########################################################################

Describe "ISO 27001 - Power BI Guest Access" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.5.15", "ISO27001:A.5.16", "ISO27002:A.5.15", "ISO27002:A.5.16" {
    BeforeAll {
    ...
    It "ISO27001.A.5.15.2: Guest user directory browsing must be disabled" {
    ...
    It "ISO27001.A.5.16.4: Invite external users permission must be disabled for guests" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.5.16.5: Guest access to organizational content must be disabled/restricted" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
...
Describe "ISO 27001 - Power BI Content Sharing" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.12", "ISO27001:A.8.13", "ISO27002:A.8.12", "ISO27002:A.8.13" {
    It "ISO27001.A.8.12.1: Publish to Web (anonymous access) must be disabled" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.8.13.1: R and Python custom visuals must be disabled" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.8.12.2: Shareable links to entire organization must be disabled" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.8.12.3: External data sharing receiver collaboration must be disabled" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
...
Describe "ISO 27001 - Power BI Protection & Authentication" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.24", "ISO27001:A.5.15", "ISO27002:A.8.24", "ISO27002:A.5.15" {
    It "ISO27001.A.8.24.1: Sensitivity labels must be enabled in Power BI" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.5.15.3: ResourceKey authentication must be blocked" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
    ...
    It "ISO27001.A.5.15.4: Service Principal API access must be disabled or restricted" {
        if (-not $script:pbiCmdAvailable -or $script:allSettings.Count -eq 0) {
            Set-ItResult -Skipped -Because "Power BI admin API not connected or not available in this session"
            return
        }
