##########################################################################
# ISO 27001 / ISO 27002 - SharePoint & OneDrive Security Tests
# Ported from M365-Assess: Collaboration/Get-SharePointSecurityConfig.ps1
# Controls: A.5.14, A.8.3, A.8.4, A.8.7, A.8.20
##########################################################################

Describe "ISO 27001 - SharePoint External Sharing" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.5.14", "ISO27001:A.8.3", "ISO27002:A.5.14", "ISO27002:A.8.3" {
    BeforeAll {
        try {
    ...
    It "ISO27001.A.5.14.3: SharePoint external sharing must not be set to 'Anyone with link'" {
    ...
    It "ISO27001.A.5.14.4: External users must not be able to re-share content" {
        if (-not $script:spoSettings) {
    ...
    It "ISO27001.A.5.14.5: Sharing domain restriction must be configured (allow or block list)" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
    ...
    It "ISO27001.A.8.3.3: Default sharing link type must be 'Specific people' (not Anyone or Organization)" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
    ...
    It "ISO27001.A.5.14.6: Guest access expiration must be enabled and set to 30 days or less" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
    ...
    It "ISO27001.A.8.3.4: Default sharing link permission must be 'View' (not Edit)" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
...
Describe "ISO 27001 - SharePoint Authentication & Access" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.20", "ISO27001:A.8.4", "ISO27002:A.8.20", "ISO27002:A.8.4" {
    BeforeAll {
        if (-not $script:spoSettings) {
    ...
    It "ISO27001.A.8.20.3: Legacy authentication protocols must be disabled in SharePoint" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
    ...
    It "ISO27001.A.8.4.1: OneDrive/SharePoint sync from unmanaged devices must be blocked" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
    ...
    It "ISO27001.A.8.4.2: Idle session timeout policy must be configured" {
        if (-not $script:spoSettings) {
            Set-ItResult -Skipped -Because "SharePoint settings not available"
            return
        }
...
Describe "ISO 27001 - SharePoint Malware Protection" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.7", "ISO27002:A.8.7" {
    BeforeAll {
    ...
    It "ISO27001.A.8.7.1: Infected file download must be blocked in SharePoint/OneDrive" {
    ...
    It "ISO27001.A.5.14.7: SharePoint/OneDrive B2B integration must be enabled for Entra-managed guest access" {
