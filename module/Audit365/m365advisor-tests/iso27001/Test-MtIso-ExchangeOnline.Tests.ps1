##########################################################################
# ISO 27001 / ISO 27002 - Exchange Online Security Tests
# Ported from M365-Assess: Exchange-Online/Get-ExoSecurityConfig.ps1
# Controls: A.5.14, A.5.16, A.8.15, A.8.20, A.8.21
##########################################################################

Describe "ISO 27001 - Exchange Online" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.20", "ISO27001:A.8.5", "ISO27002:A.8.20" {
    BeforeAll {
    ...
    It "ISO27001.A.8.20.1: Modern Authentication must be enabled" {
    ...
    It "ISO27001.A.8.20.2: SMTP AUTH must be disabled org-wide" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
...
Describe "ISO 27001 - Exchange Auditing" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.15", "ISO27002:A.8.15" {
    It "ISO27001.A.8.15.1: Exchange organization-level auditing must not be disabled" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.15.2: No mailboxes should have audit bypass enabled" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.15.3: Mailbox auditing must be enabled on all user mailboxes (sample)" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
...
Describe "ISO 27001 - Exchange Email Security" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.5.14", "ISO27001:A.8.21", "ISO27002:A.5.14", "ISO27002:A.8.21" {
    It "ISO27001.A.5.14.2: Auto-forwarding to external domains must be disabled" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.21.7: External sender tagging must be enabled to identify phishing attempts" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.21.8: Connection filter IP Allow List must be empty (no IPs bypass spam filtering)" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.21.9: Connection filter safe list must be disabled" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.21.10: No transport rules should whitelist domains by bypassing spam filtering (SCL=-1)" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
...
Describe "ISO 27001 - Exchange Mailbox Security" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.5.16", "ISO27001:A.8.2", "ISO27002:A.5.16", "ISO27002:A.8.2" {
    It "ISO27001.A.8.2.2: All shared mailbox accounts must have sign-in blocked" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.5.16.2: OWA additional storage providers must be disabled" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.5.16.3: Default role assignment policy must not allow user-installed Outlook add-ins" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
    ...
    It "ISO27001.A.8.21.11: No inbound connectors should allow unauthenticated relay (no TLS + no domain restriction)" {
        if (-not $script:exoConnected) {
            Set-ItResult -Skipped -Because "Exchange Online cmdlets not connected"
            return
        }
