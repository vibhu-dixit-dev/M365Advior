##########################################################################
# ISO 27001 / ISO 27002 - Defender for Office 365 Security Tests
# Ported from M365-Assess: Security/Defender* Checks
# Controls: A.8.13, A.8.21, A.8.22
##########################################################################

Describe "ISO 27001 - Defender Anti-Phishing & Anti-Spam" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.21", "ISO27002:A.8.21" {
    BeforeAll {
    ...
    It "ISO27001.A.8.21.1: Phishing threshold, Spoof Intel, and DMARC policies must be configured" {
    ...
    It "ISO27001.A.8.21.2: Bulk threshold, spam actions, and ZAP must be enabled" {
    ...
    It "ISO27001.A.8.21.3: Inbound spam policy must not allow sender domains globally" {
        if (-not $script:antiSpamAvailable) {
            Set-ItResult -Skipped -Because "Exchange Online / Get-HostedContentFilterPolicy is not available"
            return
        }
...
Describe "ISO 27001 - Defender Safe Links & Attachments" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.21", "ISO27001:A.8.22", "ISO27002:A.8.21", "ISO27002:A.8.22" {
    BeforeAll {
    ...
    It "ISO27001.A.8.21.4: Safe Links URL scanning and click tracking must be enabled" {
    ...
    It "ISO27001.A.8.21.5: Safe Attachments policy must be enabled and set to block malicious files" {
    ...
    It "ISO27001.A.8.21.6: Safe Attachments for SharePoint, OneDrive, and Teams must be enabled" {
...
Describe "ISO 27001 - Defender Anti-Malware & Outbound Spam" -Tag "ISO 27001", "ISO 27002", "ISO27001:A.8.22", "ISO27001:A.5.14", "ISO27002:A.8.22", "ISO27002:A.5.14" {
    BeforeAll {
    ...
    It "ISO27001.A.8.22.1: Common Attachment Filter and Malware ZAP must be enabled" {
    ...
    It "ISO27001.A.8.22.2: Malware filter must block comprehensive list of dangerous extensions" {
        if (-not $script:malwareAvailable) {
            Set-ItResult -Skipped -Because "Exchange Online / Get-MalwareFilterPolicy is not available"
            return
        }
    ...
    It "ISO27001.A.5.14.1: Automatic external forwarding must be disabled in Outbound Spam policy" {
