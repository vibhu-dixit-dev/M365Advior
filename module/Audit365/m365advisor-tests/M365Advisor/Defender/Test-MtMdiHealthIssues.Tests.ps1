BeforeDiscovery {
    ...
    $MdiAllHealthIssues | Group-Object -Property displayName, domainNamesString, sensorDNSNamesString | ForEach-Object {
        ...
        $UniqueHealthIssue.severity = $textInfo.ToTitleCase($UniqueHealthIssue.severity) # We need title case to be compatible with M365Advisor report
...
Describe "Defender for Identity health issues" -Tag "M365Advisor", "Defender",  "MDI", "MT.1059" -ForEach $MdiHealthIssuesGrouped {
    ...
    It "MT.1059.$($hash): MDI Health Issues - $($_.Name). See https://m365advisor.dev/docs/tests/MT.1059" -Tag 'MT.1059', "Severity:$($_.Group[0].severity)", $_.Name {
        ...
        $resultMdLink += "`n`n Open [Health issue - $($_.Name)](https://security.microsoft.com/identities/health-issues) in the Microsoft Defender portal."
        #endregion
