##########################################################################
# ISO 27002 / ISO 27002 - Entra ID Security & Governance Tests
# Ported from M365-Assess: Entra/*
# Controls: A.5.15, A.5.16, A.5.18, A.8.2, A.8.3, A.8.5, A.8.9
##########################################################################

Describe "ISO 27002 - Entra ID Admin Accounts" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.5.18", "ISO27002:A.5.18" {
    BeforeAll {
    ...
    It "ISO27002.5.18.1: Operational Global Administrators count must be between 2 and 4" {
    ...
    It "ISO27002.5.18.2: PIM must manage Global Administrator role (no permanent standing assignments)" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.3: Guest access reviews must be configured" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.4: Privileged roles access reviews must be configured" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.5: Global Administrator activation must require approval" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.6: Privileged Role Administrator activation must require approval" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.7: All Global Administrator accounts must be cloud-only" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.18.8: Global Administrators must not be assigned full M365 E3/E5 productivity licenses" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.3.1: Restrict non-admin users from accessing Microsoft Entra admin center" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.1: All Global Administrators must have MFA registered" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
...
Describe "ISO 27002 - Entra ID Password & MFA Settings" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.5", "ISO27002:A.5.15", "ISO27002:A.8.5", "ISO27002:A.5.15" {
    BeforeAll {
    ...
    It "ISO27002.5.15.1: Security Defaults or equivalent Conditional Access policies must be enabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.2: Telephony-based authentication methods (SMS and Voice) must be disabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.3: Email OTP authentication must be disabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.4: Custom banned password lists must be enabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.5: Smart Lockout threshold must be 10 or fewer attempts" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.6: Verified domains must have passwords set to never expire (with MFA)" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.7: Authenticator fatigue protection (number matching + app info context) must be enabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.5.8: System-Preferred MFA must be enabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
...
Describe "ISO 27002 - Entra ID Device Settings" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.2", "ISO27002:A.8.9", "ISO27002:A.8.2", "ISO27002:A.8.9" {
    BeforeAll {
    ...
    It "ISO27002.8.2.1: Microsoft Entra Join permissions must be restricted (not set to All)" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.1: Maximum device quota per user must be 15 or fewer" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.3.2: Global Administrator role must not be added as local administrator on Entra-joined devices" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.2: Local Administrator Password Solution (LAPS) must be enabled in Entra ID" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
...
Describe "ISO 27002 - Entra ID App Consent & Group Settings" -Tag "ISO 27002", "ISO 27002", "ISO27002:A.8.9", "ISO27002:A.8.3", "ISO27002:A.8.9", "ISO27002:A.8.3" {
    BeforeAll {
    ...
    It "ISO27002.8.9.3: User consent for applications must be blocked" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.4: Users must not be allowed to register custom applications" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.5.16.1: Users must not be allowed to create security groups" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.5: Non-admin users must be restricted from creating directories/tenants" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.6: Admin Consent Workflow must be enabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
    ...
    It "ISO27002.8.9.7: LinkedIn account connections must be disabled" {
        if (-not $script:graphConnected) {
            Set-ItResult -Skipped -Because "Not authenticated to Microsoft Graph"
            return
        }
